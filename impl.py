...
class Category(IdentifiableEntity):
    ...
    def hasQuartile(self) -> bool:
        return self._quartile is not None
...
class BasicQueryEngine:
    ...
    def _categories_from_df(self, df: pd.DataFrame) -> List[Category]:
        res: List[Category] = []
        for _, r in df.iterrows():
            res.append(Category(id=str(r.get("id","")), quartile=str(r.get("quartile")) if pd.notna(r.get("quartile")) else None))
        return res
